...
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory;
    ...
    native: async action => {
      ...
      if (action.action === 'share_pdf') { previewSaved(action.id, false);return typeof openUnifiedShareModal === 'function' ? openUnifiedShareModal() : shareInvoicePdfFile('voice'); }
      if (action.action === 'share_excel') { previewSaved(action.id, false);return typeof openUnifiedShareModal === 'function' ? openUnifiedShareModal() : shareSavedExcel(action.id); }
      if (action.action === 'print_invoice') { previewSaved(action.id, false);return printInvoice(); }
  ...
  function inspect(rows) {
    ...
    return { ...rows, settings, rates, today, lang: state.lang, printLang: state.printLang, printOpts: copy(state.printOpts || {}), revision: JSON.stringify([{ ...rows, settings: rows.settings.map(r => { const { assistantWorkspace, assistantLearning, ...financial } = r;return financial; }) }, rates]) };
  }
  ...
  // Conversation metadata is backed up with settings, but cannot change business records.
  async function assistantData(value) {
    if (env.locked()) throw new Error('restore_busy');
    const Agent = typeof module === 'object' && module.exports ? require('./assistant-agent.js') : globalThis.WOWAssistantAgent;
    if (!Agent) throw new Error('memory_invalid');
    const patch = {};
    if (value?.workspace) patch.assistantWorkspace = Agent.workspace(value.workspace);
    if (value?.learning) patch.assistantLearning = Agent.learning(value.learning);
    await readTransaction(await env.getDB(), 'readwrite', (rows, tx) => {
      if (env.locked()) throw new Error('restore_busy');
      const main = rows.settings.find(r => r.id === 'main') || { id: 'main' };
      tx.objectStore('settings').put({ ...main, ...patch });
    });
    Object.assign(env.getState().settings, patch);
    try { env.notify(['settings']); } catch (_) {}
    return { saved: true };
  }
  async function native(action, options = {}) {
    if (options.signal?.aborted) throw new Error('action_cancelled');
    if (env.locked()) throw new Error('restore_busy');
    if (!action || !['open', 'payments', 'preview_invoice', 'export_pdf', 'export_excel', 'share_pdf', 'share_excel', 'print_invoice'].includes(action.action)) throw new Error('action_unsupported');
    if (action.action === 'open' && !Core.pages.includes(action.page)) throw new Error('action_unsupported');
    if (action.action !== 'open') { const s = await snapshot();if (options.signal?.aborted) throw new Error('action_cancelled');if (!s.invoices.some(i => String(i.id) === String(action.id))) throw new Error('record_missing'); }
    return env.native(action);
  }
  return { snapshot, preview, commit, memory, assistantData, native };
});
