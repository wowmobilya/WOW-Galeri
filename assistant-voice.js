...
(function (root, factory) {
  ...
  const maleNames = /(?:\b(?:male|maged|tarik|naayf|hamed|hamdan|bassel|ahmet|david|mark|daniel|george|james|john|ryan|guy|eric|brian|christopher|andrew|thomas|oliver|kareem|fahrettin)\b|رجالي|ذكر|erkek)/i;
  function selectVoice(list, lang, preferred = '') {
    const available = list.filter(v => v.lang?.toLowerCase().startsWith(lang));
    return available.find(v => preferred && (v.voiceURI === preferred || v.name === preferred)) || available.find(v => maleNames.test(v.name) && !/female/i.test(v.name)) || available[0] || null;
  }
  function voices(synthesis, lang) { return (synthesis?.getVoices?.() || []).filter(v => v.lang?.toLowerCase().startsWith(lang)).sort((a,b) => Number(maleNames.test(b.name)) - Number(maleNames.test(a.name))); }
    ...
    function speak(text, lang) {
      ...
      return new Promise(resolve => {
        ...
        function next() {
          ...
          const voice = selectVoice(env.synthesis.getVoices?.() || [],lang,env.preferredVoice?.() || '');if (voice) u.voice = voice;
  ...
  return { normalize, parseControl, choose, numberAnswer, selectVoice, voices, isMale:voice=>!!voice&&maleNames.test(voice.name)&&!/female/i.test(voice.name), createSpeech, createRecognition };
});
