(function (root) {
  ...
  async function request(path, body) {
    ...
    return new Promise((resolve, reject) => {
      const requestId = 'ai-' + id();const timer = setTimeout(() => { pending.delete(requestId);reject(new Error('assistant_timeout')); }, 65000);
      pending.set(requestId, { resolve, reject, timer });
      frame.contentWindow.postMessage({ type: 'WOW_BG_BRIDGE_REQUEST', id: requestId, method: 'POST', path, body }, origin);
    });
  }
  function connect() {
  ...
  async function plan(payload) { const auth = credentials();if (!auth) throw new Error('assistant_auth_required');return request('/api/assistant/plan', { ...payload, ...auth }); }
